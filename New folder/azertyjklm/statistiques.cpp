...
{    QSqlQuery qry;
   ...
   qry.prepare("select * from client where tfete = 'birthday' ");
     if (qry.exec())
     {
         while (qry.next())
         {
     ...
     qry.prepare("select * from client where tfete = 'wedding' ");
       if (qry.exec())
       {
           while (qry.next())
           {
       ...
       qry.prepare("select * from client where tfete = 'others' ");
         if (qry.exec())
         {
             while (qry.next())
             {
    ...
    ticks << 1 << 2 << 3   ;
    labels << "birthday" << "wedding" << "others"   ;
